...
def end_effects():
    ...
    for end in ends_list:
        updated_ends_list = [v for v in ends_list]
        if end in updated_ends_list:
            updated_ends_list.remove(end)
        patterns = ortho_matching.make_patterns(starts_list,  vowels_list, updated_ends_list)
        effects, words = distribution_of_improvement(patterns, removed=True)
...
def add_end_effects():
    ...
    #for algroup in alphabet:
        updated_ends_list = [v for v in ends_list]
        updated_ends_list.append(algroup.upper())
        patterns = ortho_matching.make_patterns(starts_list,  vowels_list, updated_ends_list)
        strokes, failure = get_avg_strokes(patterns)
